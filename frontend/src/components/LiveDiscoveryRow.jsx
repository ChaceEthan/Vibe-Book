...
const LiveDiscoveryRow = ({ onStreamClick }) => {
  ...
  if (loading && activeLiveStreams.length === 0) {
    return (
      <div className="flex gap-3 overflow-x-hidden px-4 py-3">
        {[...Array(5)].map((_, index) => (
          <div key={index} className="h-32 w-[6.5rem] shrink-0 animate-pulse rounded-lg bg-white/10" />
        ))}
      </div>
    );
  }
  ...
  return (
    <motion.section
      className="relative z-30 bg-transparent py-3"
    ...
    >
      <div className="mb-2 flex items-center justify-between px-4">
        <div className="flex items-center gap-2">
          ...
          <p className="text-xs font-black uppercase tracking-wide text-white/80">Live</p>
        </div>
      ...
      {canScrollLeft && (
        <button
          type="button"
          onClick={() => scroll("left")}
          className="absolute left-1 top-1/2 z-10 hidden h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full bg-slate-950/60 text-white shadow-lg backdrop-blur transition hover:bg-slate-950/75 sm:flex"
      ...
      {canScrollRight && (
        <button
          ...
          className="absolute right-1 top-1/2 z-10 hidden h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full bg-slate-950/60 text-white shadow-lg backdrop-blur transition hover:bg-slate-950/75 sm:flex"
      ...
      <div
        ref={containerRef}
        className="scroll-smooth flex gap-2.5 overflow-x-auto px-4 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
        ...
        <AnimatePresence initial={false}>
          {activeLiveStreams.map((stream) => (
            <motion.button
              ...
              className="group relative h-32 w-[6.5rem] shrink-0 overflow-hidden rounded-lg text-left shadow-xl outline-none ring-1 ring-white/10 transition focus-visible:ring-2 focus-visible:ring-white sm:h-36 sm:w-[7.25rem]"
            ...
            >
              ...
              />
              <span className="absolute inset-[2px] overflow-hidden rounded-[0.42rem] bg-white/10">
                <SafeCoverImage
                  user={stream.creator}
                ...
                />
                <span className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-slate-950/60" />
              </span>
              ...
              <span className="absolute right-2 top-2 inline-flex items-center gap-1 rounded-full bg-slate-950/60 px-2 py-1 text-[0.58rem] font-black text-white backdrop-blur">
              ...
              <span className="absolute inset-x-2 bottom-2 flex min-w-0 items-end gap-2">
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="mt-0.5 block truncate text-[0.64rem] font-semibold text-white/70">
                    {stream.title || stream.category || "Live"}
                  </span>
                </span>
              </span>
            </motion.button>
