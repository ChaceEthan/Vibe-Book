...
const serializeProfilePost = (post, viewer = null, req = null) => {
  ...
  return {
    ...
    watchTime: Number(post.watchTime || 0),
    completionRate: Number(post.completionRate || 0),
    replays: Number(post.replays || 0),
    engagementScore: Number(post.engagementScore || 0),
    viralScore: Number(post.viralScore || 0),
    trendScore: Number(post.trendScore || 0),
    engagementVelocity: Number(post.engagementVelocity || 0),
    likes: likedBy.length,
    likeCount: likedBy.length,
    likedByViewer: Boolean(viewerId && likedBy.some((id) => id.toString() === viewerId)),
    saves: Array.isArray(post.savedBy) ? post.savedBy.length : Number(post.saves || 0),
    saveCount: Array.isArray(post.savedBy) ? post.savedBy.length : Number(post.saves || 0),
    savedByViewer: Boolean(viewerId && Array.isArray(post.savedBy) && post.savedBy.some((id) => id.toString() === viewerId)),
