# Agent setup for `vibebook`

This is a deepsec scanning workspace. Project `vibebook` was just registered
(target: `..`). Setup is incomplete — `data/vibebook/INFO.md`
...
2. **Fill in `data/vibebook/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id vibebook
pnpm deepsec process --project-id vibebook
```
